...
def test_function_request_euc():
  ...
  # Find events by content rather than hardcoded index.
  fc_event = next(
      e
      for e in events
      if e.content
      and e.content.parts
      and len(e.content.parts) >= 2
  ...
  assert fc_event.content.parts[0].function_call is not None
  assert fc_event.content.parts[1].function_call is not None
  auth_configs = list(auth_event.actions.requested_auth_configs.values())
  ...
  function_call_ids = list(auth_event.actions.requested_auth_configs.keys())

  for idx, part in enumerate(euc_event.content.parts):
    request_euc_function_call = part.function_call
    assert request_euc_function_call is not None
    assert request_euc_function_call.name == REQUEST_EUC_FUNCTION_CALL_NAME
...
def test_function_get_auth_response():
  ...
  runner = testing_utils.InMemoryRunner(agent)
  runner.run('test')
  # Find the EUC function call event by content.
  request_euc_function_call_event = next(
      e
...
def test_function_get_auth_response_partial():
  ...
  # In the workflow model, all auth responses must be sent together
  # for the execute_tools node to rerun (rerun_on_resume requires all
  # interrupts to be resolved).
  runner.run(
      new_message=types.Content(
          role='user',
          parts=[
