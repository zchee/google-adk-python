...
from google.adk.agents.llm._functions import find_matching_function_call
...
from google.adk.agents.llm._functions import merge_parallel_function_response_events
...
def test_function_call_id():
  ...
  # Find events by content (agent_state events may be interleaved).
  fc_event = next(
      e
      for e in events
      if e.content and e.content.parts and e.content.parts[0].function_call
  )
  fr_event = next(
      e
      for e in events
  ...
  assert fc_event.content.parts[0].function_call.id.startswith('adk-')
  assert fr_event.content.parts[0].function_response.id.startswith('adk-')
...
async def test_parallel_state_modifications_thread_safety():
  ...
  # Find the function response event (agent_state events may be interleaved).
